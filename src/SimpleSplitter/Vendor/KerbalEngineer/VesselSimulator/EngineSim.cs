...
namespace SimpleSplitter.Engineer.VesselSimulator
{
    ...
    public class EngineSim
    {
        ...
        internal string unsupportedBurn;
        ...
        private static void Reset(EngineSim engineSim)
        {
            ...
            engineSim.unsupportedBurn = null;
        }
									...
									LogMsg log)
        {
            ...
            engineSim.unsupportedBurn = engineMod.throttleLocked || engineMod.atmChangeFlow || engineMod.useThrustCurve
                ? "the burning engines require throttle-locked, air-dependent or variable-thrust execution, which is not supported."
                : null;
            engineSim.resourceConsumptionsForMass.Reset();
            engineSim.resourceConsumptionsForIsp.Reset();
            engineSim.resourceFlowModes.Reset();
            ...
            else
            {
                ...
                float flowModifier = GetFlowModifier(atmChangeFlow, atmCurve, 0d, velCurve, machNumber, thrustCurve, thrustCurveRatio, ref engineSim.maxMach, engineMod.flowMultCap, engineMod.flowMultCapSharpness);
                engineSim.isp = atmosphereCurve.Evaluate((float)atmosphere);
                engineSim.thrust = GetThrust(Mathf.Lerp(minFuelFlow, maxFuelFlow, GetThrustPercent(thrustPercentage)) * flowModifier, engineSim.isp);
