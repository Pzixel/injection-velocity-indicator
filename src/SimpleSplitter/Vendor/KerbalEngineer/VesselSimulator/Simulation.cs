...
namespace SimpleSplitter.Engineer.VesselSimulator {
    ...
    public class Simulation {
        ...
        private bool inFlight, inEditor;
        internal System.Threading.CancellationToken cancellation;
        ...
        public bool PrepareSimulation(LogMsg _log, List<Part> parts, double theGravity, double theAtmosphere = 0, double theMach = 0, bool dumpTree = false, bool vectoredThrust = false, bool fullThrust = false) {
            ...
            mach = theMach;
            inFlight = HighLogic.LoadedSceneIsFlight;
            ...
            for (int i = 0; i < partList.Count; ++i) {
                ...
                // Simple Splitter models main-engine burns; RCS readouts are unused.
        ...
        public Stage[] RunSimulation(LogMsg _log) {
            ...
            while (currentStage >= 0) {
                ...
                // No RCS-only readout simulation is needed by Simple Splitter.
                ...
                while (!AllowedToStage()) {
                    ...
                    if (resourceDrainTime > 0d && stepStartMass > stepEndMass && stepStartMass > 0d && stepEndMass > 0d) {
                        ...
                        // Simple Splitter: export this interval before engine depletion
                        // changes the next interval's thrust or Isp. Do not average a stage.
                        double intervalDeltaV = currentisp * Units.GRAVITY * Math.Log(stepStartMass / stepEndMass)
                            * totalStageThrust / simpleTotalThrust;
                        string unsupported = null;
                        foreach (EngineSim engine in activeEngines)
                            if (engine.unsupportedBurn != null) unsupported = engine.unsupportedBurn;
                        stage.burnSegments.Add(new EngineerSegment(stepStartMass, stepEndMass,
                            intervalDeltaV, resourceDrainTime, unsupported));
                    }
                    ...
                    if (loopCounter == 1000) {
                        ...
                        throw new InvalidOperationException("KER resource-drain iteration limit reached; refusing a partial fuel estimate.");
                    }
               ...
               if(inEditor) //this is only needed in the VAB.
            ...
            // Simple Splitter owns cleanup in a finally block, including cancellation.
